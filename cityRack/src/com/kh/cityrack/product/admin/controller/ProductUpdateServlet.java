...
public class ProductUpdateServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)){
			...
			//product_download_imgFiles에 중복된 이미지가 있을 경우 삭제
			String pcode = multipartRequest.getParameter("pcode");
			Product before = new ProductService().productGet(pcode);
			...
			if(productFilesName.get(1) == null){
				p.setP_pic1(before.getP_pic1());
				System.out.println("before");
			} else {
				p.setP_pic1(productFilesName.get(1));
				System.out.println(savePath + before.getP_pic1());
				new File(savePath + before.getP_pic1()).delete();
				System.out.println("after");
			}
			
			if(productFilesName.get(0) == null){
				p.setP_pic2(before.getP_pic2());
				System.out.println("before");
			} else {
				p.setP_pic2(productFilesName.get(0));
				System.out.println(savePath + before.getP_pic2());
				new File(savePath + before.getP_pic2()).delete();
				System.out.println("after");
			}
			
			int result = new ProductService().productUpdate(p);              
